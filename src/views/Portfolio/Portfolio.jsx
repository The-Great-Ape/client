  ...
  const getBalances = async () => {
    ...
    portfolio = portfolio
      .map((token) => {
        ...
        var price = priceData.find(
          (price) => price.mint === token.account.data.parsed.info.mint
        );
        return {
          mint: mint,
          balance: balance,
    ...
    staked = staked
      .map((stakeAccountInfo) => {
        ...
        let balance = new TokenAmount(
          userStakeInfo.depositBalance.toNumber(),
          6
        );
        balance = parseFloat(balance.format());

        let pendingReward = new TokenAmount(
          userStakeInfo.rewardDebt.toNumber(),
          6
        );
